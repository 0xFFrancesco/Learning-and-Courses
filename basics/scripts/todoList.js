import { getElements, getMetadataFromTodoEvent, getState, refreshView, saveTodoEdit, TodoAction, } from "./todoList.utils.js";
(() => {
    ...
    formEl.addEventListener("submit", (e) => {
        e.preventDefault();
        const text = new FormData(formEl).get("addTodo");
        if (text) {
            state.items.push({
                timestamp: new Date(),
                isCompleted: false,
    ...
    listEl.addEventListener("click", (e) => {
        ...
        if (index !== -1) {
            ...
            if (classes.contains(TodoAction.Save)) {
                const text = document.querySelector(`[name='editTodo-${index}']`).value;
                saveTodoEdit(index, text);
            }
