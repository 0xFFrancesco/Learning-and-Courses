...
(() => {
	...
	const callApi = (q: string) => {
		...
		return Promise.all([
			...
			.then((res) => {
				if (callId !== state.lastCallId) return;
				res.json().then((data) => {
					state.results = (data as DataOutput).recipes.map(
						(r) => r.name
					);
				});
			})
	...
	const debouncedCallback = () => {
		...
		timer = setTimeout(() => {
			...
			callApi((formData.get("search") ?? "") as string);
		}, 300);
	};
	...
	resultsEl.addEventListener("click", (e) => {
		if (state.results.length) {
			const selected = (e.target as HTMLDivElement).textContent!;
